...
Create a database named `adoptimizer`.

```
createdb adoptimizer
```
...
```bash
celery -A adoptimizer worker -l INFO --pool=solo
```

Or with celery beat (for scheduled tasks):

```bash
celery -A adoptimizer worker -l INFO -B --pool=solo
```
